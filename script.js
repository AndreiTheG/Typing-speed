...
function message() {
    ...
    let text = document.createTextNode('You wrote ' + nrCorrectWords.val + ' words out of ' + nrWords + '!');
    message.appendChild(text);
    message.style.fontSize = '30px';
    document.body.appendChild(message);
    message.style.marginTop = 300 + 'px';
...
function previousWords() {
    parag.innerHTML = '';
    for (let i = 0; i < currentIndex.val; ++i) {
        let len3 = matWords.val[i].length;
        for (let j = 0; j < len3; ++j) {
            if (isCorrectLetter[i][j] == true) {
                parag.innerHTML += '<span style="color: rgb(0, 255, 0)">' + matWords.val[i][j] + '<span>';
            } else {
                parag.innerHTML += '<span style="color: rgb(255, 0, 0)">' + matWords.val[i][j] + '<span>';
            }
        }
        parag.innerHTML += ' ';
    }
...
function verifyWord(isCorrect, lastKey) {
    let len = matWords.val[currentIndex.val].length;
    console.log(currentWord.length);
    console.log(matWords.val[currentIndex.val][index.val]);
    if (index.val < len) {
        for (let i = 0; i < index.val; ++i) {
            if (isCorrectLetter[currentIndex.val][i] == true) {
                parag.innerHTML += '<span style="color: rgb(0, 255, 0)">' + matWords.val[currentIndex.val][i] + '<span>';
            } else {
                parag.innerHTML += '<span style="color: rgb(255, 0, 0)">' + matWords.val[currentIndex.val][i] + '<span>';
            }
        }
        if (isCorrect == true && lastKey.length == 1) {
            parag.innerHTML += '<span style="color: rgb(0, 255, 0)">' + matWords.val[currentIndex.val][index.val] + '<span>';
            isCorrectLetter[currentIndex.val][index.val] = true;
        } else if (isCorrect == false && lastKey.length == 1) {
            parag.innerHTML += '<span style="color: rgb(255, 0, 0)">' + matWords.val[currentIndex.val][index.val] + '<span>';
        } else if (lastKey == 'Backspace' && currentWord.length >= 0) {
            isCorrectLetter[currentIndex.val][index.val] = false;
            parag.innerHTML += matWords.val[currentIndex.val][index.val];
        }
        for (let i = index.val + 1; i < len; ++i) {
            parag.innerHTML += matWords.val[currentIndex.val][i];
        }
    } else if (index.val == len && lastKey == 'Backspace') {
        for (let i = 0; i < len; ++i) {
            if (isCorrectLetter[currentIndex.val][i] == true) {
                parag.innerHTML += '<span style="color: rgb(0, 255, 0)">' + matWords.val[currentIndex.val][i] + '<span>';
            } else {
...
function nextWords() {
    for (let i = currentIndex.val + 1; i < nrWords; ++i) {
        parag.innerHTML += ' ';
        parag.innerHTML += matWords.val[i];
    }
}

function wrongWord(lung) {
    previousWords();
    for (let i = 0; i < lung; ++i) {
        isCorrectLetter[currentIndex.val][i] = false; 
        parag.innerHTML += '<span style="color: rgb(255, 0, 0)">' + matWords.val[currentIndex.val][i] + '<span>';
    }
    nextWords();
}
...
function pressKey(e) {
    let lung = matWords.val[currentIndex.val].length; 
    console.log(e.key);
    if (e.key.length == 1 && e.key != ' ') {
        countSpaces = 0;
        let lastKey = e.key, isCorrect = false;
        previousWords();
        if (e.key == matWords.val[currentIndex.val][index.val]) {
            isCorrect = true;
        }
        verifyWord(isCorrect, lastKey);
        currentWord += e.key;
        nextWords();
        ++index.val;
        ...
        e.preventDefault();
    } else if (e.key == ' ' && countSpaces == 0) {
        let wordLen = currentWord.length, isEqualWord = true; 
        ++countSpaces;
        if (wordLen == lung) {
            for (let i = 0; i < wordLen && isEqualWord == true; ++i) {
                if (currentWord[i] != matWords.val[currentIndex.val][i]) {
                    isEqualWord = false;
                }
            }
            if (isEqualWord == false) {
                wrongWord(lung);
            }
        } else if (wordLen < lung) {
            wrongWord(lung);
        } else {
            isEqualWord = false;
